...
import { getById } from "@/molules/tournaments/api.js";
...
export default function Team() {
  ...
  const [tournament, setTournament] = React.useState(true);
  const [
    squad,
    balance,
    players,
  ...
  React.useEffect(() => {
    const fetch = async () => {
      ...
      const tournament = await getById(team?.tournamentId);
      setPlayers(tournament?.data.players);
      setTournament(tournament?.data);
      setSquad(team?.members);
      setName(team?.name);
      setBalance(
        team?.tournamentBalance -
          team?.members.reduce((acc, val) => acc + +val?.price, 0),
      );
      setLoading(false);
    };
    fetch();
